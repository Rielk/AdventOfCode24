import { arrayAfterSplice } from "../general/arrayUtils";
...
function checkSafe(report: Array<number>, dampener: boolean): boolean {
    return findDanger(report, dampener) < 0
...
var countSafe = (withDampener: boolean) => {
    return data.reduce((sum, report) => {
        if (checkSafe(report, withDampener))
            sum++;
        return sum;
    }, 0);
}

var safeCount = countSafe(false);
var safeCountWithDampener = countSafe(true);
